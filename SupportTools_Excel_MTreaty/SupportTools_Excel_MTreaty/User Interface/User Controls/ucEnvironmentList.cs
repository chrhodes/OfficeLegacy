...
namespace SupportTools_Excel_MTreaty.User_Interface.User_Controls
{
    public partial class ucEnvironmentList : UserControl
    {
        ...
        public void PopulateListFromFile(string fileName)
        {
            ...
            {
                MessageBox.Show(string.Format("PopulateListFromFile({0}) Config file not found.", fileName));
            }
            catch (Exception ex)
            {
                MessageBox.Show(string.Format("PopulateListFromFile({0}) : {1}", fileName, ex));
            }
        }
