...
Public Class ZWorkSheet
    ...
    Public Shared Sub AddFormulas()
        'PLLog.Trace1("Enter", Globals.PROJECT_NAME)
        ...
        ''Dim val As Integer
        'Dim questionsRow As Long = Globals.cSD_QuestionIDRow
        'Dim currentProtectionMode As Boolean

        'ws = Globals.ThisAddIn.Application.ActiveSheet
        'Globals.ThisAddIn.ExcelUtil.ProtectSheet(ws, False)
        ...
        '    startDataColumn = ws.Range(Globals.cSD_StartDataColumnCell).Value
        '    endDataRow = ws.Range(Globals.cSD_EndDataRowCell).Value
        '    endDataColumn = ws.Range(Globals.cSD_EndDataColumnCell).Value
        ...
        '        "(" & _
        '        "  IF(" & Globals.cSD_TeamNameCell_RC & " = """ & Globals.cAllITString & """," & _
        '        "    IF(R" & startDataRow & "C:R" & endDataRow & "C < 6, " & _
        '        "      R" & startDataRow & "C:R" & endDataRow & "C, " & _
        '        "      """"" & _
        '        "    )," & _
        '        "    IF(" & Globals.cSD_TeamNameCell_RC & " = R" & startDataRow & "C" & Globals.cSD_TeamName_Column & ":R" & endDataRow & "C" & Globals.cSD_TeamName_Column & "," & _
        '        "      IF(R" & startDataRow & "C:R" & endDataRow & "C < 6, " & _
        '        "        R" & startDataRow & "C:R" & endDataRow & "C, " & _
        '        "        """"" & _
        '        "      )" & _
        ...
        '    "=SUM(" & _
        '    "  IF(" & Globals.cSD_TeamNameCell_RC & " = """ & Globals.cAllITString & """," & _
        '    "    IF(R" & startDataRow & "C:R" & endDataRow & "C = RC" & Globals.cSD_TeamName_Column & ", " & _
        '    "      1," & _
        '    "      0)," & _
        '    "    IF(R" & startDataRow & "C" & Globals.cSD_TeamName_Column & ":R" & endDataRow & "C" & Globals.cSD_TeamName_Column & " = " & Globals.cSD_TeamNameCell_RC & "," & _
        '    "      IF(R" & startDataRow & "C:R" & endDataRow & "C = RC" & Globals.cSD_TeamName_Column & ", " & _
        '    "        1," & _
        '    "        0)," & _
        '    "    )" & _
        ...
        '    "=SUM(IF($C$12=""" & Globals.cAllITString & """,IF(INDIRECT(""R"" & $C$13 & ""C:R"" & $C$14 & ""C"",FALSE)=$D16,1,0),IF(INDIRECT(""R"" & $C$13 & ""C:R"" & $C$14 & ""C4"",FALSE)=$C$12,IF(INDIRECT(""R"" & $C$13 & ""C:R"" & $C$14 & ""C"",FALSE) = $D16,1,0),0)))"

        '    ' This is what we want to put in.  If Cntrl-Shift-Enter from application this works.
        '    '.Range(.Cells(Globals.cQuestionIDRow - 6, startDataColumn), .Cells(Globals.cQuestionIDRow - 6, endDataColumn)).Formula = _
        ...
        '    .Range(.Cells(startDataRow, Globals.cSD_TeamName_Column), .Cells(endDataRow, Globals.cSD_TeamName_Column)).FormulaR1C1 = teamNameLookupFormula

        '    ' Walk the questions adding the statistics Array Formulas when appropriate (Statistics Row = "X")

        '    questionsRange = .Range(.Cells(Globals.cSD_QuestionIDRow, startDataColumn), .Cells(Globals.cSD_QuestionIDRow, endDataColumn))
        ...
        '        "R" & Globals.cSD_IsPrimaryQuestionColumn_Row & "C" & startDataColumn & ":" & _
        '        "R" & Globals.cSD_IsPrimaryQuestionColumn_Row & "C" & endDataColumn & ")"

        '    Dim chartCountFormula As String = "=SUM(" & _
        '        "R" & Globals.cSD_IsStatisticsColumn_Row & "C" & startDataColumn & ":" & _
        '        "R" & Globals.cSD_IsStatisticsColumn_Row & "C" & endDataColumn & ")"
        ...
        '    .Range(Globals.cSD_ChartCountCell).FormulaR1C1 = "=RC[-2]"
        '    .Range(Globals.cSD_ChartCountCell).Offset(0, -2).FormulaR1C1 = chartCountFormula
        '    .Range(Globals.cSD_QuestionCountCell).FormulaR1C1 = "=RC[-2]"
        '    .Range(Globals.cSD_QuestionCountCell).Offset(0, -2).FormulaR1C1 = questionCountFormula
        ...
        '        If rng.Offset(Globals.cSD_IsStatisticsColumnOffset, 0).Value > 0 Then
        ...
        '        If rng.Offset(Globals.cSD_ISPrimarQuestionColumnOffset, 0).Value > 0 Then
        ...
        '            Debug.Print(rng.Offset(Globals.cSD_ColumnWidthOffset, 0).Value & " " & rng.Column())

        '            If rng.Offset(Globals.cSD_ColumnWidthOffset, 0).Value < 10 Then
        '                .Range(ws.Cells(Globals.cSD_QuestionTextRow, rng.Column()), ws.Cells(Globals.cSD_QuestionTextRow, rng.Column() + 1)).Merge()
        '            End If
        '        End If

        '        If rng.Offset(Globals.cSD_IsFollowUpQuestionColumnOffset, 0).Value > 0 Then
        ...
        'Globals.ThisAddIn.ExcelUtil.ProtectSheet(ws, currentProtectionMode)
        'Globals.ThisAddIn.ExcelUtil.ScreenUpdatesOn()

        'PLLog.Trace1("Exit", Globals.PROJECT_NAME)
    End Sub
    ...
    Public Shared Function CreateWorksheet(ByVal sheetName As String) As Excel.Worksheet
        ...
        '    .Range(Globals.cSD_SurveyNameCell).Value = surveyName

        '    .Range("$B$5").Value = "Survey Period"
        '    .Range(Globals.cSD_SurveyPeriodCell).Formula = "=Survey_Period"

        '    .Range("$B$6").Value = "Questions Sheet"
        '    .Range(Globals.cSD_QuestionsSheetCell).Formula = "=$C$4 & "" Questions"""

        '    .Range("$B$7").Value = "Questions Location"
        '    .Range(Globals.cSD_QuestionsLocationCell).Formula = "=Questions_Location"

        '    .Range("$B$8").Value = "Row Width Location"
        '    .Range(Globals.cSD_ColumnWidthLocationCell).Value = "=Row_Width_Location"

        '    .Range("$B$9").Value = "Statistics Location"
        '    .Range(Globals.cSD_StatisticsLocationCell).Value = "=Statistics_Location"

        '    .Range("$B$10").Value = "Primary Questions Location"
        '    .Range(Globals.cSD_PrimaryQuestionsLocationCell).Value = "=Primary_Questions_Location"

        '    .Range("$B$11").Value = "FollowUp Questions Location"
        '    .Range(Globals.cSD_FollowUpQuestionsLocationCell).Value = "=FollowUp_Questions_Location"

        '    .Range("$B$12").Value = "Team Name"
        '    .Range(Globals.cSD_TeamNameCell).Value = "=Team_Name"

        '    .Range("$B$13").Value = "Start Data Row"
        '    .Range(Globals.cSD_StartDataRowCell).Value = startDataRow

        '    endDataRow = Globals.ThisAddIn.ExcelUtil.FindLastRow(.Range("$A$30"))

        '    .Range("$B$14").Value = "End Data Row"
        '    .Range(Globals.cSD_EndDataRowCell).Value = endDataRow
        '    ' End Data Row filled in when data loaded

        '    .Range("$B$15").Value = "Start Data Column"
        '    .Range(Globals.cSD_StartDataColumnCell).Value = startDataColumn

        '    endDataColumn = Globals.ThisAddIn.ExcelUtil.FindLastColumn(.Range("$A$30"))

        '    .Range("$B$16").Value = "End Data Column"
        '    .Range(Globals.cSD_EndDataColumnCell).Value = endDataColumn
        ...
        '    .Range("$C$23").FormulaArray = "=IF($C$12=""" & Globals.cAllITString & """,$C$14-$C$13+1,SUM(IF(INDIRECT(""R""&$C$13&""C4:R""&$C$14&""C4"",FALSE) = $C$12,1,0)))"
    ...
    Public Shared Sub FormatWorksheet()
        ...
        'currentProtectionMode = Globals.ThisAddIn.ExcelUtil.ProtectSheet(sht, False)
        'Globals.ThisAddIn.ExcelUtil.ScreenUpdatesOff()

        'With sht
        '    ' TODO: Some column name constants please.

        '    .Columns(1).ColumnWidth = Globals.cSurveyResultsRawColumn1Width
        '    .Columns(2).ColumnWidth = Globals.cSurveyResultsRawColumn2Width
        '    .Columns(3).ColumnWidth = Globals.cSurveyResultsRawColumn3Width
        '    .Columns(4).ColumnWidth = Globals.cSurveyResultsRawColumn4Width
        '    .Columns(5).ColumnWidth = Globals.cSurveyResultsRawColumn5Width

        '    questionCount = .Range(Globals.cSD_QuestionCountCell).Value
        '    chartCount = .Range(Globals.cSD_ChartCountCell).Value
        '    startDataColumn = .Range(Globals.cSD_StartDataColumnCell).Value
        '    endDataColumn = .Range(Globals.cSD_EndDataColumnCell).Value
        ...
        'Globals.ThisAddIn.ExcelUtil.ProtectSheet(sht, currentProtectionMode)
        'Globals.ThisAddIn.ExcelUtil.ScreenUpdatesOn()
    ...
    Public Shared Sub LoadSurveyDataFromMDBFile()
        ...
        'Dim inputFile As String = Globals.ThisAddIn.ExcelUtil.GetFile(Globals.cDEFAULT_SURVEY_FOLDER, "Select File Containing Survey Data", "Survey Data Files (*.MDB)|*.MDB")
