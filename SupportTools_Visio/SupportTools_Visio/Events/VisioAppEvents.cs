...
namespace SupportTools_Visio.Events
{
    class VisioAppEvents
    {
        ...
        void _VisioApplication_MarkerEvent(Microsoft.Office.Interop.Visio.Application app, int SequenceNum, string ContextString)
        {
            ...
            // If we got here from a RUNADDONWARGS("QueueMarkerEvent", ...)
            ...
            if (context.Count() > 1)
            {
                RouteShapeSheetEvent(app, SequenceNum, context);            	;
            }
            else
            {
                // Quietly ignore
        ...
        private void RouteShapeSheetEvent(Visio.Application app, int sequenceNum, string[] context)
        {
            ...
            string shapeu = context[3].Substring(8);    // "/shapeu="
            string action = context[4].Substring(8);    // "/action="

            DisplayInWatchWindow(0,string.Format("doc:>{0}<", doc));
            DisplayInWatchWindow(0,string.Format("page:>{0}<", page));
            DisplayInWatchWindow(0,string.Format("shape:>{0}<", shape));
            DisplayInWatchWindow(0,string.Format("shapeu:>{0}<", shapeu));
            DisplayInWatchWindow(0,string.Format("action:>{0}<", action));
